...
#if sizeof file is smaller than 64kb reamining bytes are initalized with 0xffffffff
instruction_memory = []


#data memory
#address range: 0x10000000 ~ 0x1000ffff
#initalize with 0xffffffff
data_memory = []
#3rd arguments
...
#NOTE: change instr to pc value 11-30

for instr in instr_list:
#while(1):

#    which_instr_to_execute = int(PC/4)

#    if(which_instr_to_execute > len(instr_list)):
#            break

#    print(which_instr_to_execute)
#    instr = instr_list[which_instr_to_execute]
#    print(instr)
#    PC = PC + 4
...
#                imm = imm * 2 + a
       ...
       #*******************************왜 하는 지 모르겟음 
        ...
        #NOTE: 11-10 does it need to be int32??
        #NOTE: 11-13 apply int32 to rd
        instr_num +=1
        ...
        if instr[17:20] == "001" or instr[17:20] == "101":
            ...
            if instr[17:20] == "001": # slli rd, rs1, shamt
                ...
                if instr[0:7] == "0000000": # srli rd, rs1, shamt
                    ...
                    #NOTE: 11-10 does not int32 because it always start with 0??
            ...
            #NOTE: 11-10 does these instructions need int32??
...
#                shamt = x[rs2] % 32   NOTE: This is invalid because of numeric issues.
        ...
        if instr[17:20] == "010":
            if instr[0:7] == "0000000": # slt rd, rs1, rs2
                ...
                #NOTE: changed 11-08 proj2_5.bin in 69th sequence slt x1, x29, x4   x1 should be 1 instead of 0
                if(x[rs1] < x[rs2]):
                    x[rd] = 1
                else:
                    x[rd] = 0
                # print(f"x[{rs1}]: {x[rs1]}, x[{rs2}]: {x[rs2]}")
        ...
        if instr[17:20] == "101":
            if instr[0:7] == "0000000":
                ...
                #NOTE: changed 11-08 does int32 function should be applied to x[rd]??
            ...
            if instr[0:7] == "0100000":
                ...
                #NOTE: does this need to be broken down like left shift??
                shamt = binary_lower_5bits_32bit(x[rs2])
                x[rd] = int32(x[rs1] >> shamt)
